...
if __name__ == '__main__':
    ...
    for filename in os.listdir(directory):
        if filename.endswith(".sol"):
            ...
            with open(contract, 'r') as raw:
                lines = raw.readlines()
                raw_code = open(directory+"_code\\"+filename, 'a+')
                open(directory+"_code\\"+filename, 'w').close()
                raw_comment = open(directory+"_comment\\"+filename, 'a+')
                open(directory + "_comment\\" + filename, 'w').close()

                multi_comment = 0
                single_comment = 0
                for line in lines:
                    line = line.lower().strip()
                    if comment_line in line and not line.startswith(comment_line):
